library(OncoSimulR)
set.seed(1) 
# no sé si esto es necesario, pero creo que sí para que los datos de
# las simulaciones sean más reproducibles
...
## demasiado (de momento el único parámetro que he elegido "razonando" es el 
## tiempo de simulación, que es el que me dejaba el ordenador sin ir como una 
## patata) y los plots van bien, aunque algunos de simulaciones tardan un poco.

## Para el segundo ejemplo fuerzo un poco el límite de genotipos permitido en el
## evalAllGenotypes, por lo que simplifico un poco el DAG (está explicado abajo)
## Las simulaciones corren y los plots salen, pero cuidado porque tiran un poco.

## Faltaría ver si intento implementar las funciones
## oncoSimulPop/oncoSimulSample para hacer alguna cosa chula.

## Tamaño inicial - 2000, sigo papers Ramón 2018 y 2015
## Tasa mutación (mu) - 1e-5, según Ramón podría ser también 1e-6

## En cuanto a los plots, he hecho por número de drivers, porque si no era una
## locura y salían ilegibles.

#############################################################
#### Ejemplo cáncer colorrectal - datos Wood et al. 2007 ####
#############################################################
...
##  Caso I - Sin forzar progresión lineal del tumor
## Esquema de la Fig. 3A, panel izquierdo

## Lo interesante aquí es que con OncoSimulR no podemos decir "no sabemos qué
## requisitos tiene esta mutación, pero sabemos que temporalmente ocurre después
## de X". Esto es lo que dice realmente la Fig.3A con respecto a TP53/EVC2 y
## PIK3CA/EPHA3: no se conocen sus requisitos pero sí se especifica que ocurren 
## después de KRAS y en estadios tardíos de la progresión tumoral respectivamente

## Opción 1 - nodos con requisitos desconocidos vienen directamente de la raíz ##
...
## In fact, Ithink the resulting graph is the one that makes the least assumptions
...
## código simulaciones, por tenerlo por si acaso (no creo que sea especialmente
## interesante, yo me centraría en la discusión) (ver abajo)
...
## Opción 2 - nodos con requisitos desconocidos vienen de KRAS
...
##  Caso II - Forzando progresión lineal del tumor
...
## ^ Algo así para decir que sabemos que en realidad esto no vale pa ná, pero 
...
## No sé si esto es relevante, pero lo dejo aunque sea para nosotros:

# cosa curiosa: con xlim = (0, 5000) salen poblaciones con 5 mutaciones --> 
# ¿muestreo demasiado "largo"? (es decir, t demasiado grande?)

# Mirando el material suplementario del paper de Ramón parece que sí estamos 
# haciendo un muestreo un poco largo si consideramos que lo que vemos es el 
# momento del diagnóstico. He decidido plotear la simulación "completa" (hasta
# t = 5000 en xlim) solo en el primer ejemplo, para enseñar lo que pasa, y
...
# el tumor, aparecen clones con más mutaciones, pero estas no parecen "fijarse"
...
# y la presión selectiva no actúa a su favor (tampoco en su contra)
...
## Lo que quiero decir ahí arriba ^ es que sé que no es correcto decir que estos
## dos módulos dependen de KRAS porque no es eso lo que dice el paper, pero que 
## es la única forma de integrar de alguna manera la información temporal que
## comentan (incluso así perdemos algo de información).

## Creo que en realidad solo usan el modelo lineal para comparar su método con 
## el de Raphael and Vandin, así que si vemos que queda muy largo lo podemos 
## quitar de la presentación (me parecería tontería quitarlo del documento que 
...
## Volvemos a tener el problema del DAG vs. progresión temporal.
...
## 1 - Modelando los módulos "huérfanos" como que vienen del Root:
...
## como vamos a enlazar con eso me parecía que podría ser coherente
## 2 - en el resto de módulos, nos quedamos con los genes que vienen en Ciriello
## 3 - junto las dos mutaciones de PTEN (D) en una sola

## Con estos cambios parece que la cosa funciona sin demasiado problema
## borro el entorno de R para hacer hueco (no sé si hace falta):
...
# con tiempo = 1000 sale muy bonico, con 5000 ya cuesta un poco hacer gráficos
